...
/*
 * RedisMgr：ChatServer 访问 Redis 的统一业务入口。
 *
 ...
 *
 * Singleton 保证整个 ChatServer 共享同一个 RedisMgr；enable_shared_from_this
...
public:
	...
	std::string acquireLock(
		...
		int acquire_timeout_seconds)
	{
		return AcquireLock(lock_name, lock_timeout_seconds, acquire_timeout_seconds);
	}
	bool releaseLock(
		const std::string& lock_name,
		const std::string& identifier)
	{
		return ReleaseLock(lock_name, identifier);
	}
	void IncreaseCount(const std::string& server_name);
	void DecreaseCount(const std::string& server_name);

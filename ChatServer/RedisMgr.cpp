...
namespace
{
	...
	constexpr const char* kCountLockName = "lockcount";
	constexpr int kLockTimeoutSeconds = 10;
	constexpr int kAcquireTimeoutSeconds = 5;
	...
	int acquire_timeout_seconds)
{
	...
	return DistLock::GetInstance().Acquire(
		connection.Get(), lock_name, lock_timeout_seconds, acquire_timeout_seconds);
}
	...
	const std::string& identifier)
{
	...
	return DistLock::GetInstance().Release(connection.Get(), lock_name, identifier);
}

void RedisMgr::IncreaseCount(const std::string& server_name)
{
	const std::string identifier = AcquireLock(
		kCountLockName, kLockTimeoutSeconds, kAcquireTimeoutSeconds);
	if (identifier.empty())
	{
		LOG_WARNING("Redis login count increase skipped because the lock was not acquired.");
		return;
	}

	std::string count_text = HGet(kLoginCountKey, server_name);
	int count = 0;
	try
	{
		count = count_text.empty() ? 0 : std::stoi(count_text);
	}
	catch (...)
	{
		count = 0;
	}
	HSet(kLoginCountKey, server_name, std::to_string(count + 1));
	ReleaseLock(kCountLockName, identifier);
}

void RedisMgr::DecreaseCount(const std::string& server_name)
{
	const std::string identifier = AcquireLock(
		kCountLockName, kLockTimeoutSeconds, kAcquireTimeoutSeconds);
	if (identifier.empty())
	{
		LOG_WARNING("Redis login count decrease skipped because the lock was not acquired.");
		return;
	}
	...
	HSet(kLoginCountKey, server_name, std::to_string(std::max(0, count - 1)));
	ReleaseLock(kCountLockName, identifier);
}

void RedisMgr::InitCount(const std::string& server_name)
{
	HSet(kLoginCountKey, server_name, "0");
}

void RedisMgr::DelCount(const std::string& server_name)
{
	HDel(kLoginCountKey, server_name);
}

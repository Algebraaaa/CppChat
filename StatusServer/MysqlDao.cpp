...
std::shared_ptr<UserInfo> MysqlDao::GetUser(int uid)
{
	...
	try
	{
		std::unique_ptr<sql::PreparedStatement> statement(
			connection.Get()->prepareStatement(
				"SELECT uid, name, email, nick, `desc` AS descs, sex, icon "
	...
	int limit)
{
	...
	try
	{
		std::unique_ptr<sql::PreparedStatement> statement(
			connection.Get()->prepareStatement(
				"SELECT a.from_uid, a.descs, a.status, u.name, u.nick, u.sex, u.icon "
				"FROM friend_apply AS a JOIN user AS u ON a.from_uid = u.uid "
	...
	std::vector<std::shared_ptr<UserInfo>>& friends)
{
	...
	try
	{
		std::unique_ptr<sql::PreparedStatement> statement(
			connection.Get()->prepareStatement(
				"SELECT u.uid, u.name, u.email, u.nick, u.`desc` AS descs, "
				"u.sex, u.icon, f.back FROM friend AS f "
				"JOIN user AS u ON f.friend_id = u.uid WHERE f.self_id = ?"));
		statement->setInt(1, self_uid);
		std::unique_ptr<sql::ResultSet> result(statement->executeQuery());
		while (result->next())
		{
...
bool MysqlDao::AddChatMsg(std::vector<std::shared_ptr<ChatMessage>>& messages)
{
	...
	try
	{
		...
		for (const auto& message : messages)
		{
			...
			statement->setString(5, message->chat_time);
			statement->setString(6, message->chat_time);
			statement->setInt(7, message->status);
			statement->executeUpdate();
